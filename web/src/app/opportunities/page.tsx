    ...
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="flex items-baseline justify-between">
        ...
        <div className="flex items-center gap-3">
          {scannedAt && (
            <span className="text-xs text-[#7898AC]">
          ...
          <button
            onClick={runScan}
            disabled={loading}
            className="text-xs px-3 py-1.5 bg-[#1E3A5F] hover:bg-[#162d4a] rounded text-white transition-colors disabled:opacity-40 font-medium"
          >
      ...
      {loading && (
        <div className="rounded-lg border border-[#BFD0DC] bg-white p-8 text-center shadow-sm">
          <p className="text-[#2D4A5C] text-sm">Analyse en cours…</p>
          <p className="text-[#7898AC] text-xs mt-1">Le premier scan peut prendre 30-60 secondes</p>
        </div>
      )}
      ...
      {!loading && !error && data.length === 0 && (
        <div className="rounded-lg border border-[#BFD0DC] bg-white p-8 text-center shadow-sm">
          <p className="text-[#2D4A5C] text-sm">Aucune opportunité au-dessus du seuil (score ≥ 6).</p>
          <p className="text-[#7898AC] text-xs mt-1">Conditions défavorables ou données incomplètes.</p>
        </div>
      )}
...
const RISK_REGIME_LABELS: Record<string, { label: string; color: string; desc: string }> = {
  ...
  "neutral":   { label: "Neutre",    color: "text-[#2D4A5C] border-[#BFD0DC] bg-[#EEF2F6]", desc: "Conditions normales" },
};
...
function MacroWidget({ macro }: { macro: MacroScan }) {
  ...
  return (
    <div className="rounded-lg border border-[#BFD0DC] bg-white p-4 shadow-sm space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="text-[10px] font-semibold text-[#7898AC] uppercase tracking-widest">Contexte macro</h2>
      ...
      <div className="flex gap-6 flex-wrap">
        {Object.entries(macro.macro).map(([name, data]) => {
          ...
          return (
            <div key={name}>
              <p className="text-[10px] text-[#7898AC] uppercase tracking-wide">{name}</p>
              <p className="text-sm font-mono text-[#0B1929] font-medium">
      ...
      {(macro.outperformers.length > 0 || macro.underperformers.length > 0) && (
        <div className="flex gap-8 text-xs pt-2 border-t border-[#BFD0DC]">
          ...
          {macro.underperformers.length > 0 && (
            <div>
              <p className="text-[#7898AC] mb-1 text-[10px] uppercase tracking-wide">Sous-performants YTD</p>
    ...
    <div className="rounded-lg border border-[#BFD0DC] bg-white p-4 hover:border-[#1E3A5F]/30 hover:shadow-sm transition-all duration-150">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-start gap-3">
          <span className="text-[#7898AC] text-sm font-mono mt-0.5 w-4 flex-shrink-0">
            {rank}.
          </span>
          <div>
            <div className="flex items-center gap-2">
              ...
              {opp.sector_group && (
                <span className="text-[10px] text-[#7898AC] border border-[#BFD0DC] rounded px-1.5 py-0.5 bg-[#EEF2F6]">
                  {opp.sector_group}
                </span>
              )}
            </div>
      ...
      {opp.has_catalyst && opp.key_headlines && opp.key_headlines.length > 0 && (
        <div className="mt-2 px-2.5 py-1.5 rounded bg-[#E2EAF0] border border-[#BFD0DC] text-xs text-[#2D4A5C]">
      ...
      <div className="mt-3 flex gap-5 text-xs text-[#7898AC] border-t border-[#BFD0DC] pt-2">
        {[
          ...
          <div key={label as string} className="text-center">
            <div className={`text-xs font-mono font-medium ${(score as number) >= 6.5 ? "text-[#1E3A5F]" : "text-[#7898AC]"}`}>
              {(score as number).toFixed(1)}
            </div>
            <div className="text-[#7898AC] text-[10px]">{label}</div>
          </div>
        ))}
      </div>
    </div>
